...
! si el problema no se resuleve con conjunto activo
if ( SiConjuntoActivo .eq. 0 ) then
!   se forman las restricciones de limites sobre los flujos de grupos de ramas
    call LimFluRes ( k, m )
endif
...
!       solo para nodos con generacion
        if ( tipnod (n) .eq. 1 .or. tipnod (n) .eq. 3 .or. tipnod (n) .eq. 4 ) then
...
!       cota superior de la variable artificial de corte
        ubMILP (  IDF + d + (i-1)*NumOferDem - 1 ) = DemFija(d,i)*SiCorte
    enddo
enddo
...
use cplex_ifaces, only: CPXgetx      , CPXgetstat    , &       !Windows (comentarizar para Linux)
                        ...
                        CPXwritemipstarts ,  CPXfopen, &       !Windows (comentarizar para Linux)
                        CPXsetlogfile, CPXreadcopymipstarts, & !Windows (comentarizar para Linux)
...
!                        CPXwritemipstarts , CPXfopen , &          !Linux (comentarizar para Windows)
!                        CPXsetlogfile,   CPXreadcopymipstarts, &  !Linux (comentarizar para Windows)
...
real*8     temp, x, slack, y, yac, Costo_Gen, fobj, fobjMILP, limit
...
if ( SiViolacion .eq. 0 .and. NumGruRamSis (sistema ) .gt. 0 .and. ite .ge. 1 .and. nomsis(sistema) .eq. 'SIN' ) then
! 	si la solucion es en variables enteras
	if ( TipoProblema .eq. 1 ) then
     	call FijaAsignacion
	endif
...
! si no se aplica el metodo de conjunto activo
if ( SiConjuntoActivo .eq. 0 ) then
    
!   guarda solucion dual de restricciones de limite superior en grupos de ramas
    k = IRTRANS
    do l = 1, numsis*SiTransmision
!       si la isla esta activa
        if ( EstadoIsla(l) .eq. 1 ) then
!           para todos los intervalos       
            do i = 1 , NTINTR
!               Para todos los grupos de ramas en el sistema
                do r = 1, NumGruRamSis ( l )
                    if ( bangruram ( r ) .eq. 1 ) then
                        br = inagruram ( r )
                        dualgrurams ( br, i ) = y ( k )
                        k = k + 1
                    endif
                enddo
            enddo
...
!   guarda solucion dual de restricciones de limite inferior en grupos de ramas
    do l = 1, numsis*SiTransmision
!       si la isla esta activa
        if ( EstadoIsla(l) .eq. 1 ) then
!           para todos los intervalos       
...
!   guarda solucion dual de restricciones de perdidas en transmision
    do l = 1, numsis*SiPerdidas
!       si la isla esta activa
        if ( EstadoIsla(l) .eq. 1 ) then
            iter = 0
!           para todas las restricciones adicionales
            do i = 1 , NumResAdi
                dualperdidas ( l, IntRestAdi(i) ) = dualperdidas ( l, IntRestAdi(i) ) + y ( k )
                if ( IntRestAdi(i) .eq. 1 ) iter = iter + 1
                if (  abs ( y ( k ) )  .gt. 0.0 ) then
                    kuenta = kuenta + 1
                    IntMulPer ( kuenta ) = IntRestAdi(i)
                    IteMulPerInt ( kuenta ) = iter
                    SisMulper ( kuenta ) = l
                    MulPerdidas ( kuenta ) = y ( k )
                endif
                k = k + 1
            enddo
        endif
...
!               apuntador al siguiente renglon
                irowMILP ( m + 1 ) = k
                write ( leti, 200 ) i
                write ( 779,* ) m, ',', '"Requerimiento del CENACE reserva regulación secundaria por sistema'//' intervalo '//trim(leti)//'"'
            else
!               para los requerimientos de reserva de regulacion
                do s = 1, NumBloRReg
!                   cota superior de reserva aceptada por CENACE
...
integer k, m, status, ibanbit, i, r, cnt, indices, sistema
integer ierror
...
!       se revisan las restricciones adicionales
        do m = NumResAsig + 1, NumResAsig + NumResAdi
            k = k + 1
...
! se escriben resultados de producción diaria en vasos a archivo RDVAAU.csv
call RDVAAU ( aa4 )
