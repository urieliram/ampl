...
param la     { CLIENTES, T };	  	#  lower limits ash percentage contents in tons of coal
param ua     { CLIENTES, T };     	#  upper limits ash percentage contents in tons of coal
param ls     { CLIENTES, T };      	#  lower limits sulfur percentage contents in tons of coal
param us     { CLIENTES, T };      	#  upper limits sulfur percentage contents in tons of coal
param r      { CLIENTES, T };      	#  per-ton per-percentage point that falls below the maximum specified percentage

#######  VARIABLES

var y { MINAS, SILOS, T, CLIENTES , T } >=0;# {i,j,k,t,tao} amount (tons) of coal shipped from mine i to siloy in period t, 
                    						# with continued shipment to customer k in period ô (where ô = t, t + 1, t + 2,
...
var y0 { SILOS, CLIENTES, T } >=0 ;	        # Amount (tons) of coal that is in initial storage at silo/, which is shipped to 
                    						# customer k in period (where tao = 1,2,3,..,TaoMax based on a three period dissipation limit)	
...
var za { CLIENTES, T } >=0;					# percentage ash content in the blended coal that is ultimately delivered to customer k in period tao

var zs { CLIENTES, T } >=0;					# percentage sulfur content in the blended coal that is ultimately delivered to customer k in period tao
...
sum { j in SILOS } sum {k in CLIENTES:(j,k) in A2 } sum {tao in 1..TaoMax} (c2[j,k] * y0[j,k,tao]) +
sum { j in SILOS} sum {t in 1..TaoMax } cS[j] * ( q0[j]  - sum { tao in T : tao<t} sum {k in CLIENTES:(j,k) in A2 } y0[j,k,tao] ) - 
sum { k in CLIENTES, tao in T } ( ( ua[k,tao]-za[k,tao]) * d[k,tao] * r[k,tao] );

###################  RESTRICCIONES

##  Balance de almacenamiento en la mina i en el Periodo del
subject to Balance_Mina {i in MINAS, del in T}: 
...
subject to Almacenamiento_Mina {i in MINAS, del in T}:
0 <= xM[i,del] <= M[i];
...
+ ( q0[j] - sum{k in CLIENTES:(j,k) in A2 } sum { tao in 1..min(del-1,TaoMax)} y0[j,k,tao]) = xS[j,del] ;
...
sum{ k in CLIENTES:(j,k) in A2 } sum { tao in 1..TaoMax } y0[j,k,tao] = q0[j];
...
za[k,tao] * d[k,tao] = sum{ j in SILOS:(j,k) in A2 } sum {i in MINAS:(i,j) in A1} sum {t in max(1,tao-2)..tao} 
alfa[i,j]*beta[i,j]*y[i,j,t,k,tao] + 
sum{ j in SILOS:(j,k) in A2 } a0[j]*y0[j,k,tao];

## Estandares de nivel de ceniza alto
subject to Nivel_Ceniza_alto {k in CLIENTES, tao in T}:
za[k,tao] <= ua[k,tao];

## Estandares de nivel de ceniza bajo
subject to Nivel_Ceniza_bajo {k in CLIENTES, tao in T}:
za[k,tao] >= la[k,tao];

## Balance de sulfuro
subject to Nivel_sulfuro {k in CLIENTES, tao in T}:
zs[k,tao] * d[k,tao] = sum{ j in SILOS:(j,k) in A2 } sum{i in MINAS:(i,j) in A1} sum{ t in max(1,tao-2)..tao} 
s[i,t]*gamma[i,j]*y[i,j,t,k,tao] + 
sum{j in SILOS:(j,k) in A2 } s0[j]*y0[j,k,tao];

## Estandares de nivel de sulfuro alto
subject to Nivel_sulfuro_alto {k in CLIENTES, tao in T}:
zs[k,tao] <= us[k,tao];

## Estandares de nivel de sulfuro bajo
subject to Nivel_sulfuro_bajo {k in CLIENTES, tao in T}:
zs[k,tao] >= ls[k,tao];