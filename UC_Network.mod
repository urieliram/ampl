...
###########  SETS
  ...
  set Re;                  ## Reserve areas
  set Da;                  ## Demand areas
  
  set AL  within {N , N};  ## Line connecting nodes n and m 
  set AD  within {Da, N};  ## Demands located at node n  
  set AG  within {N , J};  ## Generating units located at node n  
  set OG  within {J ,Re};  ## Generating units in area r
  set OD  within {Da,Re};  ## Demands in area r
...
###########  PARAMETERS
  ...
  param De   {N, T} ;     ## The hourly load forecast 
  param R    {Re,T} ;     ## The spinning reserve requirements
  ...
  param v0   {J}   ;      ## Initial status of unit j int=0
  param p0   {J}   ;      ## Power ouput of unit j in t=0
  
  param B    {N,N} ;      ## Is the negative of the series susceptance of the line (a positive constant)
...
###########  VARIABLES
 ...
 var v    {J,T} >= 0, binary; ## equals 1 if unit j is on in time period t, and 0 if it is off
 var y    {J,T} >= 0, binary; ## equals 1 if unit j starts up at the beginning of time period t, and 0 otherwise
 var z    {J,T} >= 0, binary; ## equals 1 if unit j shuts down at the beginning of time period t, and 0 otherwise
 var teta {N,T}             ; ## the voltage angle node n in period t
...
# Demand
 subject to Const_Demand{ n in N , t in T }: 
 sum { j in J:(n,j) in AG } p[ j , t ] - sum{ i in Da:(i,n) in AD } De[ i , t ] = sum{ m in N:(n,m) in AL } B[ n , m ] * ( teta[n,t] - teta[m,t] );
 
# Spinning Reserve + Demand
  subject to Const_Reserve{ r in Re , t in T }:   
  sum { j in J:(j,r) in OG } pb[ j , t ] >= sum{ i in Da:(i,r) in OD } De[ i , t ] + R[ r , t ];
...
# Transmition capacity min               
 subject to Const_TransmitionMin{ n in N , m in N:(n,m) in AL , t in T} : 
  - PT[ n , m ] <= B[ n , m ] * ( teta[ n , t ] - teta[ m , t ] );

# Transmition capacity max
 subject to Const_TransmitionMax{ n in N , m in N:(n,m) in AL , t in T} : 
 B[ n , m ] * ( teta[ n , t ] - teta[ m , t ] ) <= PT[ n , m ];
  
subject to NodoSlack{ t in T }: 
teta[ slacknode , t ] = 0;