...
!                  NoUnidades_plantaH, NumPlaH, nmxpla, PotMinUniH, PotMaxUniH, ntintr, base, AsignUH, EstadoCIUH, &
                  ...
                  PotMaxRUniH, RaRegInfH, RaRegSupH, NoUnidades_plantaH, CompSincH, NoActParH, MrreUH, TipoLec
use ProblemaAUHE
...
integer ierror, ibanbit, dummy, tempnodhid ( maxint ), nodo, subsistema, unidad, i, intervalo, modelo, &
        planta, embalse, j, k, ierror_1, ierror_2, bloque, zona, auxiliar ( maxgrure ), contador, temp1, maxpar, embalse_planta ( nmxpla ), &
        iz, sigue,  NuZonProh
...
OPEN (UNIT = 71, FILE = rut_dat_1( 1 : long_ruta )//'ZONASRESUH.csv', IOSTAT = IERROR, STATUS='OLD', RECORDSIZE = 250)
OPEN (UNIT = 195, FILE = rut_dat_1( 1 : long_ruta )//'NODOSH.csv', IOSTAT = IERROR, STATUS='OLD', RECORDSIZE = 250)
!------------------------------------------------------
...
!    write ( 1, 100 ) 'Unidad   Nombre        Propietario        Nodo     Area     Subsistema   Modelo   Max/Paros   Prioridad' 
    ...
    do while ( ierror .eq. 0 .and. len_trim(letaux) .ne. 0 )        
        ...
        read ( 71, 100, iostat = ierror ) letaux_1
        read ( 195, 100, iostat = ierror ) letaux_2
...
!            read ( letaux, * )  nombre, modelo, propietario !, dummy, tempnodhid
            read ( letaux, * )  nombre, modelo, propietario, dummy, temp1, maxpar 
            read ( letaux_1, * )  ( auxiliar ( zona ), zona = 1, maxgrure )
            read ( letaux_2, * )  ( tempnodhid ( intervalo ), intervalo = 1, ntintr )
            i = i + 1
...
!                                                 unidadh_area ( unidad ), nomsis ( IslaGenH ( unidad ) ), MOUNHI ( unidad ), NumMaxParoUH ( unidad ), PrioridadH ( unidad )
    ...
    bmensaje = fecha_Ej//' '//NomEjecu//'LEC ERROR DE LECTURA ARCHIVO UNIH.csv/ZONASRESUH.csv'
    ...
    if ( ierror .eq. 0 ) then
        ...
        do while ( ierror .eq. 0 .and. len_trim(letaux) .ne. 0 )     
	        ...
	        if ( ierror .eq. 0 .and. len_trim(letaux) .ne. 0 ) then
                i = i + 1
                read ( letaux, * )  LimIEnerEmb ( i ), LimSEnerEmb ( i ), RestEnergia ( i ) 
        ...
        ierror = 0
        Call FechaEjecucion (fecha_Ej)
        bmensaje = fecha_Ej//' '//NomEjecu//'LEC ERROR DE LECTURA ARCHIVO LIMENEREMB.csv'
        Call Mensaje_AuSeg ( ierror, ibanbit, BMensaje )
        write(*,*) '1'
!       Se ecribe resultado de semaforos
        call EscSemaforosError
...
!                read ( letaux, * )  NOMPLAH ( i ), dummy, embalse_planta ( i ), NoUnidades_plantaH ( i )
                read ( letaux, * )  ( CostoOporPH ( i, intervalo ), intervalo = 1, ntintr )
!                write ( 1, 400 ) i, NOMPLAH ( i ), NOMEMB ( embalse_planta ( i ) ), NoUnidades_plantaH ( i )
        ...
        ierror = 0
        Call FechaEjecucion (fecha_Ej)
!        bmensaje = fecha_Ej//' '//NomEjecu//'LEC ERROR DE LECTURA ARCHIVO PLAH.csv/COSTOPHID.csv'
        bmensaje = fecha_Ej//' '//NomEjecu//'LEC ERROR DE LECTURA ARCHIVO COSTOPHID.csv'
        Call Mensaje_AuSeg ( ierror, ibanbit, BMensaje )
        write(*,*) '1'
!       Se ecribe resultado de semaforos
        call EscSemaforosError
...
!                CostoOporUH ( unidad, : ) = CostoOporPH ( planta, : ) + (PrioridadH(unidad)-1)*epsilon
    ...
    OPEN (UNIT = 54, FILE = rut_dat_1( 1 : long_ruta )//'LIUNIH.csv', IOSTAT = IERROR_1, STATUS='OLD', RECORDSIZE = 250)

    ! Abre archivo de datos de generadores
    OPEN (UNIT = 55, FILE = rut_dat_1( 1 : long_ruta )//'LSUNIH.csv', IOSTAT = IERROR, STATUS='OLD', RECORDSIZE = 250)
    ...
    ! Lee información hasta encontrar fin de información
        do while ( ierror .eq. 0 .and. len_trim(letaux) .ne. 0 .and. ierror_1 .eq. 0 .and. len_trim ( letaux_1 ) .ne. 0 )       
            ...
            !Hacer para todoas las unidades hidro
            do unidad = 1, NumUniHid
                if ( corresph ( unidad ) .eq. i ) then
                    if ( ierror .eq. 0 .and. len_trim(letaux) .ne. 0 .and. ierror_1 .eq. 0 .and. len_trim(letaux_1) .ne. 0 ) then
                        read ( letaux, * )  ( PotMinUniH ( unidad, intervalo ), intervalo = 1, ntintr )
                        read ( letaux_1, * )  ( PotMaxUniH ( unidad, intervalo ), intervalo = 1, ntintr )
                        bloque = ntintr / 24
                        do k = 1, bloque
                            write ( 1, 500 ) 'intervalo:', 24 * k - 23, 'a', 24 * k
        ...
        ierror = 0
        Call FechaEjecucion (fecha_Ej)
        bmensaje = fecha_Ej//' '//NomEjecu//'LEC ERROR DE LECTURA ARCHIVO LIUNIH.csv / LSUNIH.csv'
        Call Mensaje_AuSeg ( ierror, ibanbit, BMensaje )
        write(*,*) '1'
!       Se ecribe resultado de semaforos
        call EscSemaforosError
        ...
        ! Lee información hasta encontrar fin de información
            do while ( ierror .eq. 0 .and. len_trim(letaux) .ne. 0 .and. ierror_1 .eq. 0 .and. len_trim ( letaux_1 ) .ne. 0 )       
                ...
                do unidad = 1, NumUniHid
                    if ( corresph ( unidad ) .eq. i ) then
                        if ( ierror .eq. 0 .and. len_trim(letaux) .ne. 0 .and. ierror_1 .eq. 0 .and. len_trim(letaux_1) .ne. 0 ) then
                            read ( letaux, * )  ( PotMinRUniH ( unidad, intervalo ), intervalo = 1, ntintr )
                            read ( letaux_1, * )  ( PotMaxRUniH ( unidad, intervalo ), intervalo = 1, ntintr )
                            bloque = ntintr / 24
                            do k = 1, bloque
                                write ( 1, 500 ) 'intervalo:', 24 * k - 23, 'a', 24 * k
            ...
            ierror = 0
            Call FechaEjecucion (fecha_Ej)
            bmensaje = fecha_Ej//' '//NomEjecu//'LEC ERROR DE LECTURA ARCHIVO LIRUNIH.csv / LSRUNIH.csv'
            Call Mensaje_AuSeg ( ierror, ibanbit, BMensaje )
            write(*,*) '1'
    !       Se ecribe resultado de semaforos
            call EscSemaforosError
...
!        PotMaxRUniH = PotMaxRUniH / Base
    ...
    ! Abre archivo de datos de generadores
    OPEN (UNIT = 54, FILE = rut_dat_1( 1 : long_ruta )//'COMPSH.csv', IOSTAT = IERROR_1, STATUS='OLD', RECORDSIZE = 250)
    ...
    ! Lee información hasta encontrar fin de información
        do while ( ierror .eq. 0 .and. len_trim(letaux) .ne. 0 .and. ierror_1 .eq. 0 .and. len_trim ( letaux_1 ) .ne. 0 )       
            ...
            do unidad = 1, NumUniHid
                if ( corresph ( unidad ) .eq. i ) then
                    if ( ierror .eq. 0 .and. len_trim(letaux) .ne. 0 .and. ierror_1 .eq. 0 .and. len_trim(letaux_1) .ne. 0 ) then
                        read ( letaux, * )  ( CompSincH ( unidad, intervalo ), intervalo = 1, ntintr )
                        bloque = ntintr / 24
                        do k = 1, bloque
                            write ( 1, 500 ) 'intervalo:', 24 * k - 23, 'a', 24 * k
        ...
        ierror = 0
        Call FechaEjecucion (fecha_Ej)
        bmensaje = fecha_Ej//' '//NomEjecu//'LEC ERROR DE LECTURA ARCHIVO COMPSH.csv'
        Call Mensaje_AuSeg ( ierror, ibanbit, BMensaje )
        write(*,*) '1'
!       Se ecribe resultado de semaforos
        call EscSemaforosError
    ...
    ! Lee información hasta encontrar fin de información
        do while ( ierror .eq. 0 .and. len_trim(letaux) .ne. 0 )        
            ...
            do unidad = 1, NumUniHid
                if ( corresph ( unidad ) .eq. i ) then
                    if ( ierror .eq. 0 .and. len_trim(letaux) .ne. 0 ) then
                        read ( letaux, * )  ( AsignUH ( unidad, intervalo ), intervalo = 1, ntintr )
                        bloque = ntintr / 24
                        do k = 1, bloque
                            write ( 1, 500 ) 'intervalo:', 24 * k - 23, 'a', 24 * k
        ...
        ierror = 0
        Call FechaEjecucion (fecha_Ej)
        bmensaje = fecha_Ej//' '//NomEjecu//'LEC ERROR DE LECTURA ARCHIVO ASIGNH.csv'
        Call Mensaje_AuSeg ( ierror, ibanbit, BMensaje )
        write(*,*) '1'
!       Se ecribe resultado de semaforos
        call EscSemaforosError
    ...
    ! Lee información hasta encontrar fin de información
        do while ( ierror .eq. 0 .and. len_trim(letaux) .ne. 0 )        
            ...
            do unidad = 1, NumUniHid
                if ( corresph ( unidad ) .eq. i ) then
                    if ( ierror .eq. 0 .and. len_trim(letaux) .ne. 0 ) then
                        read ( letaux, * )  ( DispoUH ( unidad, intervalo ), intervalo = 1, ntintr )
                        bloque = ntintr / 24
                        do k = 1, bloque
                            write ( 1, 500 ) 'intervalo:', 24 * k - 23, 'a', 24 * k
        ...
        ierror = 0
        Call FechaEjecucion (fecha_Ej)
        bmensaje = fecha_Ej//' '//NomEjecu//'LEC ERROR DE LECTURA ARCHIVO DISPOH.csv'
        Call Mensaje_AuSeg ( ierror, ibanbit, BMensaje )
        write(*,*) '1'
!       Se ecribe resultado de semaforos
        call EscSemaforosError
...
!                        write ( 1, 700 ) unidad, nombunih ( unidad ), EstadoCIUH ( unidad ), NoActParH ( unidad )
        ...
        ierror = 0
        Call FechaEjecucion (fecha_Ej)
        bmensaje = fecha_Ej//' '//NomEjecu//'LEC ERROR DE LECTURA ARCHIVO UNIHCI.csv'
        Call Mensaje_AuSeg ( ierror, ibanbit, BMensaje )
        write(*,*) '1'
!       Se ecribe resultado de semaforos
        call EscSemaforosError
    ...
    ! Lee información hasta encontrar fin de información
        do while ( ierror .eq. 0 .and. len_trim(letaux) .ne. 0 )        
            ...
            do unidad = 1, NumUniHid
                if ( correspH ( unidad ) .eq. i ) then
                    if ( ierror .eq. 0 .and. len_trim(letaux) .ne. 0 ) then
                        read ( letaux, * )  emergencia, regulacion, ( ZonProh ( unidad, contador ), contador = 1, maxzonproh * 2) 
        ...
        ierror = 0
        Call FechaEjecucion (fecha_Ej)
        bmensaje = fecha_Ej//' '//NomEjecu//'LEC ERROR DE LECTURA ARCHIVO RAMPASH.csv'
        Call Mensaje_AuSeg ( ierror, ibanbit, BMensaje )
        write(*,*) '1'
!       Se ecribe resultado de semaforos
        call EscSemaforosError
...
PotMaxRUniH = PotMaxRUniH*Base
    ...
    ! Abre archivo de datos de generadores
    OPEN (UNIT = 60, FILE = rut_dat_1( 1 : long_ruta )//'POTRESRO10H.csv', IOSTAT = IERROR_1, STATUS='OLD', RECORDSIZE = 250)

    ! Abre archivo de datos de generadores
    OPEN (UNIT = 61, FILE = rut_dat_1( 1 : long_ruta )//'PRERESRO10H.csv', IOSTAT = IERROR, STATUS='OLD', RECORDSIZE = 250)

    if ( ierror .eq. 0 .and. ierror_1 .eq. 0 ) then
    ! Lee información hasta encontrar fin de información
        do while ( ierror .eq. 0 .and. len_trim(letaux) .ne. 0 .and. ierror_1 .eq. 0 .and. len_trim ( letaux_1 ) .ne. 0 )       
            read ( 60, 100, iostat = ierror ) letaux
            read ( 61, 100, iostat = ierror_1 ) letaux_1
            i = i + 1
            !Hacer para todoas las unidades de rango continuo
            do unidad = 1, NumUniHid
                if ( corresph ( unidad ) .eq. i ) then
                    if ( ierror .eq. 0 .and. len_trim(letaux) .ne. 0 .and. ierror_1 .eq. 0 .and. len_trim(letaux_1) .ne. 0 ) then
                        read ( letaux, * )  ( OferResR10H ( unidad, intervalo ), intervalo = 1, ntintr )
                        read ( letaux_1, * )  ( PreVenResR10H ( unidad, intervalo ), intervalo = 1, ntintr )
                        bloque = ntintr / 24
                        do k = 1, bloque
                            write ( 1, 500 ) 'intervalo:', 24 * k - 23, 'a', 24 * k
        ...
        ierror = 0
        Call FechaEjecucion (fecha_Ej)
        bmensaje = fecha_Ej//' '//NomEjecu//'LEC ERROR DE LECTURA ARCHIVO POTRESRO10H.csv / PRERESRO10H.csv'
        Call Mensaje_AuSeg ( ierror, ibanbit, BMensaje )
        write(*,*) '1'
!       Se ecribe resultado de semaforos
        call EscSemaforosError
!       algoritmo no termina bien
    ...
    ! Abre archivo de datos de generadores
    OPEN (UNIT = 62, FILE = rut_dat_1( 1 : long_ruta )//'POTRESROSUH.csv', IOSTAT = IERROR_1, STATUS='OLD', RECORDSIZE = 250)

    ! Abre archivo de datos de generadores
    OPEN (UNIT = 63, FILE = rut_dat_1( 1 : long_ruta )//'PRERESROSUH.csv', IOSTAT = IERROR, STATUS='OLD', RECORDSIZE = 250)
    ...
    ! Lee información hasta encontrar fin de información
        do while ( ierror .eq. 0 .and. len_trim(letaux) .ne. 0 .and. ierror_1 .eq. 0 .and. len_trim ( letaux_1 ) .ne. 0 )       
            ...
            do unidad = 1, NumUniHid
                if ( corresph ( unidad ) .eq. i ) then
                    if ( ierror .eq. 0 .and. len_trim(letaux) .ne. 0 .and. ierror_1 .eq. 0 .and. len_trim(letaux_1) .ne. 0 ) then
                        read ( letaux, * )  ( OferResRxH ( unidad, intervalo ), intervalo = 1, ntintr )
                        read ( letaux_1, * )  ( PreVenResRxH ( unidad, intervalo ), intervalo = 1, ntintr )
                        bloque = ntintr / 24
                        do k = 1, bloque
                            write ( 1, 500 ) 'intervalo:', 24 * k - 23, 'a', 24 * k
        ...
        ierror = 0
        Call FechaEjecucion (fecha_Ej)
        bmensaje = fecha_Ej//' '//NomEjecu//'LEC ERROR DE LECTURA ARCHIVO POTRESROSUH.csv / PRERESROSUH.csv'
        Call Mensaje_AuSeg ( ierror, ibanbit, BMensaje )
        write(*,*) '1'
!       Se ecribe resultado de semaforos
        call EscSemaforosError
!       algoritmo no termina bien
    ...
    ! Abre archivo de datos de generadores
    OPEN (UNIT = 64, FILE = rut_dat_1( 1 : long_ruta )//'POTRESRESEH.csv', IOSTAT = IERROR_1, STATUS='OLD', RECORDSIZE = 250)

    ! Abre archivo de datos de generadores
    OPEN (UNIT = 65, FILE = rut_dat_1( 1 : long_ruta )//'PRERESRESEH.csv', IOSTAT = IERROR, STATUS='OLD', RECORDSIZE = 250)

    if ( SiResRegDis .eq. 1 ) then
!       Abre archivo de datos de reserva distribuida
        OPEN (UNIT = 66, FILE = rut_dat_1( 1 : long_ruta )//'MINRESRESEH.csv', IOSTAT = IERROR, STATUS='OLD', RECORDSIZE = 250)
    endif
    ...
    ! Lee información hasta encontrar fin de información
            ...
            do unidad = 1, NumUniHid
                if ( corresph ( unidad ) .eq. i ) then
                    if ( ierror .eq. 0 .and. len_trim(letaux) .ne. 0 .and. ierror_1 .eq. 0 .and. len_trim(letaux_1) .ne. 0 .and. ierror_2 .eq. 0 .and. len_trim(letaux_2) .ne. 0 ) then
                        read ( letaux, * )  ( OferResRegH ( unidad, intervalo ), intervalo = 1, ntintr )
                        read ( letaux_1, * )  ( PreVenResRegH ( unidad, intervalo ), intervalo = 1, ntintr )
                        if ( SiResRegDis .eq. 1 ) then
                            read ( letaux_2, * )  ( MrreUH ( unidad, intervalo ), intervalo = 1, ntintr )
                        endif
                        bloque = ntintr / 24
                        do k = 1, bloque
                            write ( 1, 500 ) 'intervalo:', 24 * k - 23, 'a', 24 * k
        ...
        ierror = 0
        Call FechaEjecucion (fecha_Ej)
        bmensaje = fecha_Ej//' '//NomEjecu//'LEC ERROR DE LECTURA ARCHIVO POTRESRESEH.csv / PRERESRESEH.csv / MINRESRESEH.csv'
        Call Mensaje_AuSeg ( ierror, ibanbit, BMensaje )
        write(*,*) '1'
!       Se ecribe resultado de semaforos
        call EscSemaforosError
!       algoritmo no termina bien
    ...
    ! Abre archivo de datos de generadores
    OPEN (UNIT = 66, FILE = rut_dat_1( 1 : long_ruta )//'POTRESNR10H.csv', IOSTAT = IERROR_1, STATUS='OLD', RECORDSIZE = 250)

    ! Abre archivo de datos de generadores
    OPEN (UNIT = 67, FILE = rut_dat_1( 1 : long_ruta )//'PRERESNR10H.csv', IOSTAT = IERROR, STATUS='OLD', RECORDSIZE = 250)
    ...
    ! Lee información hasta encontrar fin de información
        do while ( ierror .eq. 0 .and. len_trim(letaux) .ne. 0 .and. ierror_1 .eq. 0 .and. len_trim ( letaux_1 ) .ne. 0 )       
            ...
            do unidad = 1, NumUniHid
                if ( corresph ( unidad ) .eq. i ) then
                    if ( ierror .eq. 0 .and. len_trim(letaux) .ne. 0 .and. ierror_1 .eq. 0 .and. len_trim(letaux_1) .ne. 0 ) then
                        read ( letaux, * )  ( OferResNR10H ( unidad, intervalo ), intervalo = 1, ntintr )
                        read ( letaux_1, * )  ( PreVenResNR10H ( unidad, intervalo ), intervalo = 1, ntintr )
                        bloque = ntintr / 24
                        do k = 1, bloque
                            write ( 1, 500 ) 'intervalo:', 24 * k - 23, 'a', 24 * k
        ...
        ierror = 0
        Call FechaEjecucion (fecha_Ej)
        bmensaje = fecha_Ej//' '//NomEjecu//'LEC ERROR DE LECTURA ARCHIVO POTRESNR10H.csv / PRERESNR10H.csv'
        Call Mensaje_AuSeg ( ierror, ibanbit, BMensaje )
        write(*,*) '1'
!       Se ecribe resultado de semaforos
        call EscSemaforosError
!       algoritmo no termina bien
    ...
    ! Abre archivo de datos de generadores
    OPEN (UNIT = 68, FILE = rut_dat_1( 1 : long_ruta )//'POTRESNRSUH.csv', IOSTAT = IERROR_1, STATUS='OLD', RECORDSIZE = 250)

    ! Abre archivo de datos de generadores
    OPEN (UNIT = 69, FILE = rut_dat_1( 1 : long_ruta )//'PRERESNRSUH.csv', IOSTAT = IERROR, STATUS='OLD', RECORDSIZE = 250)
    ...
    ! Lee información hasta encontrar fin de información
        do while ( ierror .eq. 0 .and. len_trim(letaux) .ne. 0 .and. ierror_1 .eq. 0 .and. len_trim ( letaux_1 ) .ne. 0 )       
            ...
            do unidad = 1, NumUniHid
                if ( corresph ( unidad ) .eq. i ) then
                    if ( ierror .eq. 0 .and. len_trim(letaux) .ne. 0 .and. ierror_1 .eq. 0 .and. len_trim(letaux_1) .ne. 0 ) then
                        read ( letaux, * )  ( OferResNRxH ( unidad, intervalo ), intervalo = 1, ntintr )
                        read ( letaux_1, * )  ( PreVenResNRxH ( unidad, intervalo ), intervalo = 1, ntintr )
                        bloque = ntintr / 24
                        do k = 1, bloque
                            write ( 1, 500 ) 'intervalo:', 24 * k - 23, 'a', 24 * k
        ...
        ierror = 0
        Call FechaEjecucion (fecha_Ej)
        bmensaje = fecha_Ej//' '//NomEjecu//'LEC ERROR DE LECTURA ARCHIVO POTRESNRSUH.csv / PRERESNRSUH.csv'
        Call Mensaje_AuSeg ( ierror, ibanbit, BMensaje )
        write(*,*) '1'
!       Se ecribe resultado de semaforos
        call EscSemaforosError
